              ...
              <div className="p-6 text-gray-800">
                <div className="overflow-auto">
                  <table className="w-full text-sm text-left rtl:text-right text-gray-500">
                    ...
                    <tbody>
                      {banks.data.map((bank) => {
                        console.log(bank); // Check the bank object
                        return (
                          <tr className="bg-white border-b" key={bank.id}>
                            <td className="px-3 py-2">{bank.id}</td>
                            <td className="px-3 py-2">
                              <img src={bank.image_path} style={{ width: 60 }} alt="Bank" />
                            </td>
                            <th className="px-3 py-2 text-gray-800">
                              {bank.id ? (
                                <Link href={route('bank.show', { bank: bank.id })}>
                                  {bank.name}
                                </Link>
                              ) : (
                                <span>{bank.name}</span> // Fallback display
                              )}
                            </th>
                            ...
                            <td className="px-3 py-2 text-nowrap">
                              {bank.id ? (
                                ...
                                <span className="text-gray-400">Edit</span> // Optional fallback display
                              )}
                              <button
